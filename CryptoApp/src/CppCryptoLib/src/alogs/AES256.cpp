...
bool AES256::encryptFile(const FileData *fileData) {
  if (!fileData || fileData->Key->empty() || fileData->Iv->empty()) {
    std::cerr << "AES256: Null pointer in FileData" << std::endl;
    return false;
  }
  ...
  if (!infile.is_open()) {
    std::wcerr << L"Failed to open input file: " << fileData->OriginalFilePath->wstring() << std::endl;
    return false;
  }
  ...
  if (1 != EVP_EncryptInit_ex(ctx, EVP_aes_256_cbc(), nullptr, fileData->Key->data(), fileData->Iv->data())) {
...
bool AES256::decryptFile(const FileData *fileData) {
  ...
  std::ofstream outfile(fileData->DecryptedFilePath->wstring(), std::ios::binary);
  if (!outfile.is_open()) {
  ...
  if (1 != EVP_DecryptInit_ex(ctx, EVP_aes_256_cbc(), nullptr, fileData->Key->data(), fileData->Iv->data())) {
