...
public void handle(HttpExchange exchange) throws IOException {
    if ("POST".equals(exchange.getRequestMethod())) {
        ...
        try {
            ...
            os.close();
            System.out.println("Replaced entry with FileID: " + new String(data.FileID, StandardCharsets.UTF_8));
        } catch (Exception e) {
            e.printStackTrace();
            ...
            os.write(response.getBytes());
            os.close();
            System.out.println("Failed to replace entry with FileID: " + new String(data.FileID, StandardCharsets.UTF_8));
        }
    } else {
